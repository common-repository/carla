<?php
/*
Plugin Name: Carla Wordpress Security Plugin
Plugin URI: https://www.leclerc-web.fr
...
$resultats_count = $wpdb->get_results("SELECT COUNT(*) as nbr_log FROM {$wpdb->prefix}ip_log");
...
foreach ($resultats_count as $nbr) {
	
	if(($nbr->nbr_log < 50) && (!isset($donnees)) || ($enregistrement == true)){	
	
		$resultats = $wpdb->get_results("INSERT INTO {$wpdb->prefix}ip_log (ip, date_log) VALUES ('$ip', NOW())");    
		
	}elseif(($nbr->nbr_log >= 50) && (!isset($donnees)) || ($enregistrement == true)){
		
		$resultats = $wpdb->get_results("TRUNCATE TABLE {$wpdb->prefix}ip_log");
		$resultats = $wpdb->get_results("INSERT INTO {$wpdb->prefix}ip_log (ip, date_log) VALUES ('$ip', NOW())");
