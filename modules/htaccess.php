...
$comparaison_versionphp  = stristr($content_access, $versionphp_code);

		
if($comparaison_conf == true && $comparaison_versionphp == true && $comparaison_nav == true && $comparaison_htaccess == true && $comparaison_include == true && !file_exists($wpconfigchemin) && ($number_maj <= 0) && ($SSL_cert[0] == "https") && ($wpdb->prefix != "wp_")) {
...
/* ---------------------------------------------------------------------------------- */
	...
	elseif($htaccess == "versionphp") {
		
		if($comparaison_versionphp == false){
			insert_with_markers($htaccesschemin, "carla_versionPHP", $versionphp_code);
			$valids['include'] = '<div class="alert alert-success">
									  <strong>'.__('PHP version is hidden !','carla').'</strong>
								  </div>'; 
		}else{
			$errors['include'] = '<div class="alert alert-danger">
									<strong>'.__('Warning ! ','carla').'</strong>'.__('The PHP version is visible !','carla').'</div>';
		}
	}
				...
				<div class="card">
					<div class="card-block">
					  ...
					  <?php
						...
						if($comparaison_versionphp == false){
							echo '<div class="check_prob"><i class="fa fa-check" aria-hidden="true"></i> '.__('The PHP version is visible !','carla').'</div>';
						}else{
							echo '<div class="check_ok"><i class="fa fa-check" aria-hidden="true"></i> '.__('PHP version is hidden !','carla').'</div>';	
						}	
						...
						if($wpdb->prefix == "wp_") {
							echo '<div class="check_prob"><i class="fa fa-check" aria-hidden="true"></i> '.__('you should change the database prefix (wp_) !','carla').'</div>';
						}else{
							echo '<div class="check_ok"><i class="fa fa-check" aria-hidden="true"></i> '.__('You have changed "wp_" prefix, it\'s ok  !','carla').'</div>';	
						}	
						
						if(file_exists($wpconfigchemin)) {					
							if(($chmod != "0440") && ($chmod != "0400")) {
								echo '<div class="check_prob"><i class="fa fa-check" aria-hidden="true"></i> '.__('You should change the permissions (chmod) of your wp-config.php file.You are currently in '.$chmod.' ! it is advisable to replace by 0400 or 0440','carla').'</div>';
							}else{
								echo '<div class="check_ok"><i class="fa fa-check" aria-hidden="true"></i> '.__('Chmod is OK, wp-config file is protected ! '.$chmod.'','carla').'</div>';	
							}	
						}
					 ?>
					</div>
							...
							<form method="POST" action="">
								<div class="selectform">
									<select name="htaccess">
										...
										<option   value="versionphp" <?php if($comparaison_versionphp == true){
											echo 'disabled'; }?>><?php echo __('Hide PHP version in HTTP header','carla');?></option>
									</select>
								</div>
								<input  type="submit" name="securiser" value="<?php echo __('To secure !','carla');?>" <?php if($comparaison_conf == true && $comparaison_nav == true && $comparaison_htaccess == true && $comparaison_include == true && $comparaison_versionphp == true) {echo 'disabled';} ?> class="btn btn-primary marge_input"/>
						   </form>
						</p>
						<div class="row">
							<div class="col-sm-2">
